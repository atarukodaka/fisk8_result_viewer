class CompetitionUpdater
  def initialize(parser_type: nil, verbose: false)
    ...
    @parser = CompetitionParser.new(parser_type)
    @verbose = verbose
  end
  ...
  def update_category_result(competition, category, result_url)
    ...
    ActiveRecord::Base.transaction {
      ...
      @parser.parse(:category_result, result_url).each do |result_parsed|
  ...
  def update_score(competition, category, segment, score_url, result_url, additionals = {})
    segment_results = @parser.parse(:segment_result, result_url)
    segment_type = (segment =~ /SHORT/ || segment =~ /RHYTHM/) ? :short : :free

    @parser.parse(:score, score_url).each do |sc_parsed|
      ActiveRecord::Base.transaction {
